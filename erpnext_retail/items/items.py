...
@frappe.whitelist()
def add_items(doc, item_attribute, company, item_price_array, item_name, item_group, article, brand, hsn, purchase_invoice_doc):
	...
	try:
		...
		for variant_item_array in variant_item_cunks:
			count = enqueue_multiple_variant_creation(template.name, json.dumps({item_attribute: variant_item_array}))
		...
		items = frappe.get_all('Item', filters=[['Item Variant Attribute', 'variant_of', '=', template.name], ['Item Variant Attribute', 'attribute', '=', item_attribute], ['Item Variant Attribute', 'attribute_value', 'in', attribute_array]],)
